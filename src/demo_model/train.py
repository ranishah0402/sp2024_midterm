...
val_dataloader = DataLoader(val_dataset, shuffle=False, batch_size=28)
...
try:
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5)   # pick one from torch.optim
except Exception as e:
...
def train(loger, train_dataloader, model, optimizer, device, processor):
    ...
    for idx, batch in progress_bar:
        #print("in the loop, going through batches in training")
